...
export class TechniciansListComponent implements OnInit {
  ...
  loadTechnicians(): void {
    ...
    this.techniciansService.list(this.activeOnly).subscribe({
      next: (technicians) => {
  ...
  get filteredTechnicians(): TechnicianGridRow[] {
    let rows = this.technicians;

    if (this.workshopFilter) {
      rows = rows.filter((t) => t.workshop_id === this.workshopFilter);
    }

    const term = this.searchTerm.trim().toLowerCase();
    if (term) {
      rows = rows.filter((t) => {
        const haystack = [t.full_name, t.national_id, t.specialty, t.phone]
          .filter(Boolean)
          .join(' ')
          .toLowerCase();
  ...
  onFileSelected(event: Event): void {
    ...
    importFileWithMapping<TechnicianImportRow>(file, TECHNICIAN_IMPORT_MAP)
      .then((result) => {
        const { resolved, unresolved } = resolveTechnicianForeignKeys(result.valid, workshopIdByName);
  ...
  exportExcel(): void {
    exportToExcel(this.filteredTechnicians, this.excelColumns(), 'technicians-export');
  }
